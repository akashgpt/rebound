...
const double rot_period = INFINITY;//cd rotation period in secs (e.g.: ~7hrs for Chariklo)
...
//Files written during program
    ...
    fclose(fprop);
    	...
    	// Calculate the time taken by reb_integrate
    //~ clock_t t;
    //~ t = clock();
...
//Simulation parameters and initialization
	...
	//~ struct reb_particle cb = {0};
	//~ cb.m 		= 0.0E24;
    //~ cb.r		= 126.E3;        // Radius of cb!
	//~ reb_add(r, cb);
...
void heartbeat(struct reb_simulation* r){
	...
	for (int i=0;i<N;i++){
		...
		//~ fprintf(fprop, "%d\t%.15e\t%.15f\t%.20f\n",i+1,r->t, r_ptcl, ( (v_ptcl*v_ptcl/2.) - (G*cb_mass/(r_ptcl)) - (omega_cb*omega_cb*r_ptcl*r_ptcl/2.) ) );
		//~ fprintf(fprop, "%d\t%.15e\t%.15f\t%.15f\t%.15f\n",i+1,r->t, r_ptcl, v_ptcl, ( (v_ptcl*v_ptcl/2.) + V_ell - (omega_cb*omega_cb*r_ptcl*r_ptcl/2.) ) );
    ...
    fprintf(fvel, "\n");
    fprintf(facc, "\n");
    //~ fprintf(fprop, "%.15e\t%.15e\t%.15e\t%.15e\n",r->t, L_all, Lx, Ly, Lz, PE_all, KE_all, C_all );
